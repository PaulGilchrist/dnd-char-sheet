...
// ---------------------------------------------------------------------------
// Speed halved display — unique to this file
// ---------------------------------------------------------------------------
describe('CharSummary - Speed Halved Display', () => {
    ...
    it('does not show speed halved message when slow spell is not active', () => {
        render(
            <CharSummary
                playerStats={mockPlayerStats}
                campaignName={mockCampaignName}
                exhaustionLevel={0}
                conditionEffects={{ speedHalved: false }}
            />
        );
        expect(screen.queryByText(/Speed halved from Slow/)).not.toBeInTheDocument();
    });
});
