// Handler callback tests for summon/animation modals in CharActionModals
...
describe('CharActionModals — summon handler callbacks', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  // ── Animate Dead handler ──

  describe('Animate Dead handler', () => {
    it('calls confirmAnimateDead with zombieCount and skeletonCount on confirm', async () => {
      const setModalState = vi.fn();
      render(<CharActionModals
        {...createBasePropsWithPopup({ setModalState })}
  ...
  // ── Create Undead handler ──

  describe('Create Undead handler', () => {
    it('calls confirmCreateUndead with ghoulCount on confirm', async () => {
      const setModalState = vi.fn();
      render(<CharActionModals
        {...createBasePropsWithPopup({ setModalState })}
  ...
  // ── Summon Spirit handler ──

  describe('Summon Spirit handler', () => {
    it('calls confirmSummonSpirit with variantName on confirm', async () => {
      const setModalState = vi.fn();
      render(<CharActionModals
        {...createBasePropsWithPopup({ setModalState })}
  ...
  // ── Wild Magic Surge with spellModalState ──

  describe('Wild Magic Surge with spellModalState', () => {
    ...
    it('calls both setModalState and setSpellModalState on close for wildMagicSurgeModal', () => {
      const setModalState = vi.fn();
      const setSpellModalState = vi.fn();
      render(<CharActionModals
        {...createBaseProps()}
        ...
        setSpellModalState={setSpellModalState}
      />);
      // The WildMagicSurgeModal close handler calls both setters
      // Our mock doesn't have a close button, but the component closes on its own
      expect(screen.getByTestId('wild-magic-surge-modal')).toBeInTheDocument();
    });
  });

  // ── mergedModalState memoization ──

  describe('mergedModalState memoization', () => {
    it('combines modalState and spellModalState so spellModalState props override', () => {
      const setSpellModalState = vi.fn();
      render(<CharActionModals
        {...createBaseProps()}
        modalState={{ someProp: 'fromModalState' }}
        spellModalState={{ wildMagicSurgeModal: { surgeTable: [], mode: 'roll' } }}
        setModalState={vi.fn()}
        setSpellModalState={setSpellModalState}
      />);
      // Both modalState and spellModalState props should be merged
      expect(screen.getByTestId('wild-magic-surge-modal')).toBeInTheDocument();
    });
  });
});
