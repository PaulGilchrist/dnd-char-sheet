...
describe('Map - player context menu', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        clearRuntimeState('test-campaign');
        MockEventSource.instances.length = 0;
    ...
    it('removes a player through the player context menu', async () => {
        mockState.mapData = createMockMapData({
            players: [{ id: 'player1', name: 'Thorin', gridX: 5, gridY: 5 }],
        });
        const { container } = await act(async () => renderMap());
        const circle = container.querySelector('.creature-group circle');
        ...
        expect(handlers.setMapData).toHaveBeenCalledTimes(1);
        const updater = handlers.setMapData.mock.calls[0][0];
    ...
    it('closes the player context menu via its close button', async () => {
        mockState.mapData = createMockMapData({
            players: [{ id: 'player1', name: 'Thorin', gridX: 5, gridY: 5 }],
        });
        const { container } = await act(async () => renderMap());
