...
vi.mock('../../services/automation/handlers/spells/forcecageHandler.js', () => ({ removeForcecageEffect: vi.fn() }));
vi.mock('../../services/automation/handlers/spells/mazeHandler.js', () => ({ removeMazeEffect: vi.fn() }));
...
vi.mock('../encounter/MonsterCardModal.jsx', () => ({ default: ({ monster }) => <div data-testid="monster-card-modal" data-monster={monster?.index || 'none'}>{monster?.name || 'none'}</div> }));
...
vi.mock('./CreatureCard.jsx', () => ({ default: ({ creature, isActive, isLocalhost, onHpChange, onInitiativeChange, onTargetChange, onRollConditionSave, onBreakCondition, onOpenEffectAdder, onRollConcentrationSave, onBreakConcentration, onRemoveNpc, onNpcClick, onNameChange, allCreatures, overlays, _campaignNpcs }) => (
    <div data-testid={`creature-card-${creature.name}`} className={`creature-card ${creature.type} ${isActive ? 'active' : ''}`}>
        ...
        {(creature.type !== 'player' || creature.wildShapeSource || creature.polymorphSource || creature.shapechangeSource) && (
...
describe('Initiative - NPC Click & Condition Save Special Paths', () => {
    let props;
    ...
    beforeEach(() => {
        ...
        props = {
            characters: [
                { name: 'Alice', computedStats: { hitPoints: 20, currentHitPoints: 20, armorClass: 15, abilities: [{ name: 'Intelligence', score: 16 }, { name: 'Wisdom', score: 14 }, { name: 'Charisma', score: 12 }], languages: ['Common', 'Elvish'] } },
            ],
    ...
    describe('handleNpcClick - polymorph paths', () => {
        it('should open monster card for wild shape source creature', async () => {
            const cs = { round: 1, creatures: [{ name: 'Alice', type: 'player', wildShapeSource: 'Alice', beastName: 'Bear', beastIndex: 'bear' }] };
            vi.mocked(loadCombatSummary).mockResolvedValue(cs);
            vi.mocked(getCombatSummary).mockReturnValue(cs);
            vi.mocked(loadMonsters).mockResolvedValue([{ index: 'bear', name: 'Brown Bear', armor_class: 11, hit_points: 34, ability_scores: { str: 19, dex: 10, con: 16, int: 3, wis: 13, cha: 7 }, saving_throws: { str: { modifier: 6 }, dex: { modifier: 2 } }, actions: [], size: 'large', type: 'beast', challenge_rating: 1 }]);
            await act(async () => { render(<Initiative {...props} />); });
            await waitFor(() => { expect(screen.queryByTestId('creature-card-Alice')).toBeInTheDocument(); });

            await act(async () => {
                const avatar = screen.getByTestId('npc-click-Alice');
        ...
        it('should open monster card for polymorph source creature', async () => {
            const cs = { round: 1, creatures: [{ name: 'Alice', type: 'player', polymorphSource: 'Alice', polymorphBeast: { index: 'bear', challengeRating: 1 }, ac: 14 }] };
            vi.mocked(loadCombatSummary).mockResolvedValue(cs);
            vi.mocked(getCombatSummary).mockReturnValue(cs);
            vi.mocked(loadMonsters).mockResolvedValue([{ index: 'bear', name: 'Brown Bear', armor_class: 11, hit_points: 34, ability_scores: { str: 19, dex: 10, con: 16, int: 3, wis: 13, cha: 7 }, saving_throws: {}, actions: [], size: 'large', type: 'beast', challenge_rating: 1 }]);
            await act(async () => { render(<Initiative {...props} />); });
            await waitFor(() => { expect(screen.queryByTestId('creature-card-Alice')).toBeInTheDocument(); });
        ...
        it('should open monster card for shapechange source creature', async () => {
            const cs = { round: 1, creatures: [{ name: 'Alice', type: 'player', shapechangeSource: 'Alice', shapechangeForm: { index: 'bear', challengeRating: 1 }, ac: 14 }] };
            vi.mocked(loadCombatSummary).mockResolvedValue(cs);
            vi.mocked(getCombatSummary).mockReturnValue(cs);
            vi.mocked(loadMonsters).mockResolvedValue([{ index: 'bear', name: 'Brown Bear', armor_class: 11, hit_points: 34, ability_scores: { str: 19, dex: 10, con: 16, int: 3, wis: 13, cha: 7 }, saving_throws: {}, actions: [], size: 'large', type: 'beast', challenge_rating: 1 }]);
            await act(async () => { render(<Initiative {...props} />); });
        ...
        it('should open monster card for direct monster index', async () => {
            const cs = { round: 1, creatures: [{ name: 'Goblin', type: 'npc', monsterIndex: 'goblin', ac: 15, currentHp: 7, size: 'small', speed: 30, saveBonuses: { dex: 4 }, resistances: '', immunities: '', actions: [] }] };
            vi.mocked(loadCombatSummary).mockResolvedValue(cs);
            vi.mocked(getCombatSummary).mockReturnValue(cs);
            vi.mocked(loadMonsters).mockResolvedValue([{ index: 'goblin', name: 'Goblin', armor_class: 15, hit_points: 7, ability_scores: { str: 8, dex: 14, con: 10, int: 8, wis: 8, cha: 8 }, saving_throws: { dex: { modifier: 4 } }, actions: [], size: 'small', type: 'humanoid', challenge_rating: 0.25 }]);
            await act(async () => { render(<Initiative {...props} />); });
        ...
        it('should fallback to getMonsterData when no runtime data', async () => {
            const cs = { round: 1, creatures: [{ name: 'Ogre', type: 'npc' }] };
            vi.mocked(loadCombatSummary).mockResolvedValue(cs);
            vi.mocked(getCombatSummary).mockReturnValue(cs);
            vi.mocked(getMonsterData).mockResolvedValue({ index: 'ogre', name: 'Ogre' });
            await act(async () => { render(<Initiative {...props} />); });
            await waitFor(() => { expect(screen.queryByTestId('creature-card-Ogre')).toBeInTheDocument(); });

            await act(async () => {
                const avatar = screen.getByTestId('npc-click-Ogre');
                fireEvent.click(avatar);
            });
    ...
    describe('handleApplyEffect', () => {
        it('should not apply effect when not localhost', async () => {
            ...
            const addBtn = screen.queryByTestId('effect-add-Alice');
