...
vi.mock('../../services/items/lootGenerator.js', () => ({
    generateLootFromCombatSummary: vi.fn(async () => ({ lootEntries: ['Gold coins (100)', 'Silver sword'], totalEncounterXp: 200 })),
}));
...
describe('Initiative - Loot Handlers', () => {
    let props;
    ...
    describe('handleGenerateLoot', () => {
        it('should generate loot and display it', async () => {
            vi.mocked(loadCombatSummary).mockResolvedValue({ round: 1, creatures: [{ name: 'Alice', type: 'player' }, { name: 'Goblin', type: 'npc', currentHp: 0 }] });
    ...
    describe('handleAwardLoot', () => {
        it('should show award loot button when loot is generated with XP', async () => {
            vi.mocked(loadCombatSummary).mockResolvedValue({ round: 1, creatures: [{ name: 'Alice', type: 'player' }, { name: 'Goblin', type: 'npc', currentHp: 0 }] });
            await act(async () => { render(<Initiative {...props} />); });
            await waitFor(() => { expect(screen.queryByTestId('creature-card-Alice')).toBeInTheDocument(); });
    ...
    describe('handleClearLoot', () => {
        it('should clear loot suggestions', async () => {
            ...
            // Clear button appears when there's loot
            await act(async () => {
    ...
    describe('handleAutoBreakCondition', () => {
        it('should automatically break condition when localhost', async () => {
            vi.mocked(loadCombatSummary).mockResolvedValue({ round: 1, creatures: [{ name: 'Alice', type: 'player' }] });
            await act(async () => { render(<Initiative {...props} />); });
            await waitFor(() => { expect(screen.queryByTestId('creature-card-Alice')).toBeInTheDocument(); });
        });
