...
describe('useSpellMetamagicFlow — upcast maxTargets parsing', () => {
  ...
  it('parses maxTargets from Hold Monster upcast_at_slot_level', () => {
  ...
  it('parses maxTargets from Charm Monster upcast_at_slot_level', () => {
    const { result } = renderHook(() =>
      useSpellMetamagicFlow(makePlayerStats(), 'TestCampaign', vi.fn())
    );
    ...
    act(() => {
      result.current.gateMetamagic(makeSpell({
        ...
        level: 4,
        upcastLevel: 6,
        upcast_at_slot_level: { '6': '5 targets' },
      }));
    });

    expect(result.current.pendingCharmMonster).not.toBeNull();
    expect(result.current.pendingCharmMonster.maxTargets).toBe(5);
  });

  it('parses maxTargets from Banishment upcast_at_slot_level', () => {
    const { result } = renderHook(() =>
      useSpellMetamagicFlow(makePlayerStats(), 'TestCampaign', vi.fn())
    );
    ...
    act(() => {
      result.current.gateMetamagic(makeSpell({
        ...
        level: 4,
        upcastLevel: 5,
        upcast_at_slot_level: { '5': '2 targets' },
      }));
    });

    expect(result.current.pendingBanishment).not.toBeNull();
    expect(result.current.pendingBanishment.maxTargets).toBe(2);
  });
});
