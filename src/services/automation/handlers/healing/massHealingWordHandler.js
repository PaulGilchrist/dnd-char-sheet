...
export async function handle(action, playerStats, campaignName, _mapName) {
    ...
    const slotLevel = auto?.slotLevel || action.spell?.level || 3;
    ...
    if (eligible.length === 0) {
        return {
            type: 'popup',
            payload: { type: 'automation_info', name: MASS_HEALING_WORD_NAME, description: `${MASS_HEALING_WORD_NAME}: No creatures in combat.` },
        };
    }

    if (eligible.length <= maxTargets) {
        return confirmMassHealingWord(action, playerStats, campaignName, eligible.map(c => c.name), healExpression, maximize, bonusHeal, bonusDetails, slotLevel);
    }
...
export async function confirmMassHealingWord(action, playerStats, campaignName, selectedTargetNames, healExpression, maximize, bonusHeal, bonusDetails, _slotLevel) {
