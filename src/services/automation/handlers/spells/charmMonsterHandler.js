...
export async function handle(action, playerStats, campaignName, _mapName) {
    ...
    let targetNames;
    const charmMonsterTargets = action.metaCtx?.charmMonsterTargets;
    if (charmMonsterTargets && Array.isArray(charmMonsterTargets) && charmMonsterTargets.length > 0) {
        targetNames = charmMonsterTargets;
    } else {
    ...
    const charmMonsterAdvantages = action.metaCtx?.charmMonsterAdvantages || {};

    for (const targetName of targetNames) {
        const targetCreature = cs.creatures.find(c => c.name === targetName);
        const isTargetNpc = targetCreature && targetCreature.type !== 'player';
        const targetAdvantage = charmMonsterAdvantages[targetName] || auto.advantage || false;
