const auditRules = {
    ...
    auditKnownSpells: (playerStats) => {
        // Warn if the player has not choosen the correct number of known/prepared spells
        if (playerStats.spellAbilities && playerStats.class.name != 'Druid' && playerStats.class.name != 'Paladin') { // Druids and Paladins know all spells
            const allowed = playerStats.spellAbilities.prepared_spells || playerStats.spellAbilities.spells_known;
            return {
