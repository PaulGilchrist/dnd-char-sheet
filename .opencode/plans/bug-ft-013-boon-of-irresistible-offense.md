...
The target's resistance is being applied normally. Damage is halved despite the feat being present on the character. The attack log shows "Resistant Ooze resists Bludgeoning" and the target's HP does not decrease by the full damage amount.

## Steps to Reproduce

1. Create a 2024 ruleset character (e.g., Human Fighter level 20)
2. Add the "Boon Of Irresistible Offense" feat to the character
3. Create an NPC with B/P/S resistance (e.g., "Resistant Ooze" with `damageResistances: ["Bludgeoning", "Piercing", "Slashing"]`)
4. Add both to the initiative and set the character to target the NPC
5. Attack the NPC with a Bludgeoning damage attack (e.g., Unarmed Strike)
6. Observe the attack result shows "resists" and the NPC's HP does not decrease by the full damage amount

## Likely Location

The automation is defined in:
- **Manifest:** `docs/automations-manifest.json` line 990-998 (FT-013)
...
- FT-014 in the manifest appears to be a duplicate entry for the same feat
