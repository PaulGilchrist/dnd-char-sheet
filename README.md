...
| 🧬 **Transformation Tracking** | Polymorph, True Polymorph, Animal Shapes, Shapechange — track transformed states, revert on 0 HP or concentration break |
| 🐾 **Summoned Creature Management** | Full lifecycle for spell-summoned creatures — track duration, auto-cleanup on concentration break or caster death, integrate with combat |
| 🧭 **Passive Skills** | Auto-computed Passive Perception, Passive Investigation, and Passive Insight displayed in senses section |
| 📸 **Campaign Snapshots** | Create backup snapshots of entire campaigns with one click, restore from any snapshot |
| 🎲 **Random Events During Travel** | 6 event types (combat, discovery, hazard, NPC encounter, weather change, navigation) triggered during hex travel with terrain-specific tables |
| 🛠️ **Campaign Admin Panel** | Clear change data (HP, conditions, spell slots, death saves, target effects), clear campaign log, full reset, rename campaign |
...
- **Backup with snapshots** — Create full campaign snapshots with one click, restore from any snapshot to undo mistakes or preserve milestones
